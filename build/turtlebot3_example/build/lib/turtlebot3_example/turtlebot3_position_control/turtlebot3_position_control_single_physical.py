...
class Turtlebot3PositionControl(Node):
    ...
    def __init__(self):
        ...
        ************************************************************"""
        ...
        self.yr = 0
        self.thr = 0
        self.Xr = np.array([[self.xr], [self.yr], [self.thr]])
        ######## initialize plot #########
...
###########################################################################################################
        ...
        ************************************************************"""
        ...
        #self.odom_sub = self.create_subscription(Odometry, 'odom', callback_odom, qos)
    ...
    *******************************************************************************"""
    ...
    def optitracktb30_callback(self, msg):
        ...
        _, _, self.thr = self.euler_from_quaternion(msg.pose.orientation)
    ...
    def start_mpc(self):
        ...
        # and (mpciter < sim_tim / T)
            ...
            self.t0, self.u0 = shift(self.T, self.t0, self.u)
            self.x0 = self.Xr
            #print(self.x0)
            ...
            self.twisttb30.linear.x=self.u_cl[mpciter,0]
            print('linear',self.u_cl[mpciter,0])
            self.twisttb30.angular.z=self.u_cl[mpciter,1]
            print('angular',self.u_cl[mpciter,1])
...
##########################################################################################################################          
    def unicycle_simulation(self):
        ...
        self.x_cur = self.Xr[0]
        self.y_cur = self.Xr[1]
        self.d_cur = self.Xr[2]
        #print(self.x_cur)
        #print(self.y_cur)
